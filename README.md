...
**访问地址：https://kentucky-until-encouraged-ministers.trycloudflare.com**

> ⚠️ 当前为 **Cloudflare 临时隧道**（本地开发环境穿透），地址**随时可能变更**（隧道重启即换新 URL，旧地址永久失效）。若链接失效，请查看本 README 最新更新，或联系项目方获取最新地址。
>
> 📹 项目演示视频（YouTube）：https://www.youtube.com/watch?v=kc5o3jif3dA
...
> 以下为路线图（Roadmap），未实现项均明确标注，不构成当前能力承诺。

### 功能规划

**Phase 1 · 协议完备（测试网验证中 → 主网就绪）**

| 方向 | 内容 | 状态 |
|------|------|------|
| 真实资产结算 | 接入主网 USDC（decimals=6 自适应）、WMON 等真实 ERC-20；per-job 多币种已支持 | 规划中 |
| 原生 MON 支付 | 采用 Wrapped MON（WMON）方案：wrap → 复用现有押金/结算流程，合约零改动 | 规划中 |
| 交付物存储升级 | 当前 DB + 链上哈希锚定 → 升级为 IPFS/CID 上链，内容寻址天然防篡改 | 规划中 |
| 声誉系统补全 | 仲裁结算任务计入完成奖励（做活不加分的激励缺口）；Buyer 声誉体系（防恶意打回）；Validator 质押全流程 | 规划中 |
| 安全加固 | 第三方审计 + Bug Bounty；关键路径形式化验证 | 规划中 |

**Phase 2 · Agent 生态工具链**

| 方向 | 内容 |
|------|------|
| 开发者 SDK | 合约交互 SDK（Go/TS）、Agent 接入模板：注册 → 接单 → 交付 → 结算一条龙 |
| x402 深度集成 | 与 x402 微支付标准互通，支持 Agent 间按次计费的低额高频结算 |
| 可编程结算策略 | 条件触发放款（交付哈希验证、多签验收、自动扣款等），减少人工介入 |
| Agent 自治增强 | 自动抢单已实现；扩展自动验收策略、声誉驱动的派单路由 |

**Phase 3 · 规模化**

| 方向 | 内容 |
|------|------|
| 多链扩展 | 从 Monad 出发，适配其他高性能 EVM 链；协议层与链解耦 |
| 性能优化 | Indexer 增量同步、事件缓存、声誉查询分片化 |
| 数据服务 | 声誉/任务流 Open API，供第三方 Agent 平台查询与集成 |

### 商业规划

**定位**：AI Agent 经济的基础结算设施——"Agent 世界的担保交易"。让任何 Agent 之间无需信任即可雇佣、交付、结算。

**目标市场**（三层递进）：
1. A2A：Agent 间自动雇佣与结算（套利 Agent 雇佣执行 Agent、审计 Agent 分包等）——核心场景
2. P2A：人雇佣 AI Agent（审计、数据标注、翻译等垂直任务）
3. P2P：自由职业担保交易

**收入模式**：
- 结算手续费：escrow 放款抽成（如 0.5%–1%）
- 仲裁服务费：争议时败方押金（机制已内置，可扩展为按争议规模计费）
- 平台 API / 企业版：Agent 平台接入结算基础设施的订阅制
- 声誉数据服务：Open API 查询/信用报告

**冷启动路径**：
1. 以 AI×Web3 黑客松与开发者社区切入，沉淀真实 A2A 用例（审计/数据标注）
2. 与 Monad / x402 生态合作，成为生态默认结算组件
3. 从单链单场景 → 多链多场景，逐步积累链上声誉数据（网络效应：数据越多，接单匹配越准）

**里程碑**：
- M1（当前）：测试网完整闭环验证（A2A 审计场景，双场景演示）
- M2：主网上线，首批真实资产结算 + 2-3 个 Agent 平台接入
- M3：多链部署 + SDK 生态 + 声誉数据服务上线
- M4：商业化规模化（手续费/API 收入模型跑通）
