...
contract X402IntegrationTest is Test {
    ...
    function testFallbackERC20WhenNoFacilitator() public {
        ...
        vm.prank(buyer);
        uint256 jobId = jobNoFacilitator.createJob(AGENT_ID, 0, deadline, address(0));
    ...
    function testDualPathOnSameContract() public {
        ...
        // Job 1: x402 path
        vm.prank(buyer);
        uint256 jobId1 = jobWithFacilitator.createJob(AGENT_ID, 0, deadline, address(0));
        vm.prank(buyer);
        jobWithFacilitator.fundViaToken(jobId1, 0, bytes("receipt-1"));

        // Job 2: ERC-20 fallback path on the SAME contract
        vm.prank(buyer);
        uint256 jobId2 = jobWithFacilitator.createJob(AGENT_ID, 0, deadline, address(0));
        vm.prank(buyer);
    ...
    function testReceiptIsOpaqueBytes() public {
        ...
        vm.prank(buyer);
        uint256 jobId3 = jobWithFacilitator.createJob(AGENT_ID, 0, deadline, address(0));
    ...
    function testRevertX402WithNonZeroAmount() public {
        uint64 deadline = uint64(block.timestamp) + DEADLINE_OFFSET;
        vm.prank(buyer);
        uint256 jobId = jobWithFacilitator.createJob(AGENT_ID, 0, deadline, address(0));
